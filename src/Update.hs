...
updateAttrPath log mergeBase updateEnv@UpdateEnv {..} attrPath = do
  ...
  successOrFailure <- runExceptT $ do
    ...
    buildOutcome <- Nix.buildForUpdate attrPath
    let buildValidationNote = buildOutcomeNote buildOutcome
    when (not $ T.null buildValidationNote) (lift . log $ buildValidationNote)
    ...
    publishPackage log updateEnv' oldSrcUrl newSrcUrl attrPath result opReport prBase rewriteMsgs buildValidationNote (isJust existingCommitMsg)
...
publishPackage ::
  ...
  Bool ->
  ExceptT Text IO ()
publishPackage log updateEnv oldSrcUrl newSrcUrl attrPath result opReport prBase rewriteMsgs buildValidationNote branchExists = do
...
buildOutcomeNote :: Nix.BuildOutcome -> Text
buildOutcomeNote Nix.BuiltNormally = ""
buildOutcomeNote Nix.BuiltWithAllowUnsupportedSystem =
  "Package metadata excludes the update worker host platform; validation retried with `allowUnsupportedSystem = true`."
...
prMessage ::
  ...
  Text ->
  Text
prMessage updateEnv metaDescription metaHomepage metaChangelog rewriteMsgs releaseUrl compareUrl resultCheckReport commitRev attrPath maintainers resultPath opReport cveRep cacheTestInstructions buildValidationNote nixpkgsReviewMsg =
  ...
  let metaHomepageLine =
      ...
      buildValidationNoteLine =
        if buildValidationNote == T.empty
          then ""
          else "- " <> buildValidationNote
      checksDone = T.intercalate "\n" $ filter (not . T.null) ["- built on NixOS", buildValidationNoteLine, resultCheckReport]
   in [interpolate|
