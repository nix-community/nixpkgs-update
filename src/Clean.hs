...
import Utils (Version, setupNixpkgs, succeded, canFail)
...
fixSrcUrl packageName oldVersion newVersion derivationFile attrPath oldSrcUrl = do
  ...
  forResult <- runExceptT $ runExceptRT $ forM_ filteredDownloads $ \downloadUrl -> do
    ...
    lift $ cmd "sed" "-i" ("s|" <> oldUrl <> "|" <> newUrl <> "|") derivationFile
    lift $ cmd "grep" "-q" ("url = \"" <> newUrl <> "\";") derivationFile

    whenM (lift $ succeded $ cmd "grep" "-q" ("url = \"" <> newUrl <> "\";") derivationFile) $ do
      hash <- lift $ canFail $ cmd "nix-prefetch-url" "-A" (attrPath <> ".src")
      succeedT hash
